...
from PlatformDebris import PlatfromDebris
...
class Map(object):
    ...
    def loadWorld_11(self):
        tmx_data = load_pygame("world/1-1/W11.tmx")
        ...
        for layer in tmx_data.visible_layers:
            for y in range(tmx_data.height):
                for x in range(tmx_data.width):
                    ...
                    image = tmx_data.get_tle_image(x, y, layer_num)
                    ...
                    if image is not None:
                        ...
                        if layer.name == 'Foreground':
                            ...
                            #22 ID is a question block, so in that case e should laod all it's images
                            if tileID == 22:
                                image = (
                                    image,
    ...
    def spawn_score_text(self, x, y, score=None):
        if score is None:
            self.text_object.append(Text(str(self.score_for_killing_mob), 16, (x, y)))
            self.score_time = pg.time.get_tick()
            if self.score_for_killing_mob < 1600:
                self.score_for_killing_mob *= 2

        else:
            self.text_object.append(TExt(str(score), 16, (x, y)))
    ...
    def entity_collisions(self, core):
        if not core.get_map().get_player().unkillable:
            for mob in self.mobs:
                mob.check_collisions_with_player(core)
    ...
    def try_spawn_mobs(self, core):
        ...
        def player_death(self, core):
            ...
            if self.get_player(). numofLives == 0:
                ...
                self.get_event()start_kill(core,gaem_over=False)
        ...
        def update(self, core):
            ...
            for whizzing in self.projectiles:
